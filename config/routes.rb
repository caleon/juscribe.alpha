...
ActionController::Routing::Routes.draw do |map|
  ...
  map.with_options :requirements => { :year => /\d{4}/, :month => /\d{1,2}/, :day => /\d{1,2}/, :article_id => regex_for(:article, :permalink), :user_id => regex_for(:user, :nick) } do |art|
    art.with_options :controller => 'clips' do |cl|
      # Article clips
    ...
    # Article -> Comments
    art.with_options :controller => 'comments' do |cm|
      # Article comments
    ...
    art.with_options :controller => 'permissions', :requirements => { :comment_id => regex_for(:comment, :id) } do |pm|
    ...
    # Article -> Permissions
    art.with_options :controller => 'permissions' do |pm|
    ...
    # Article -> Pictures
    art.with_options :controller => 'pictures' do |picture|
    ...
    art.with_options :controller => 'permissions', :requirements => { :picture_id => regex_for(:picture, :id) } do |ap|
