module ArticlesHelper
  ...
  def format_article(article, opts={})
    ...
    escape_code_html(formatted)       # But this needs to escape all HTML tags within CODE/PRE
  ...
  def escape_code_html(hpricot)
    (hpricot/"pre, code").each do |code_el|
      ...
      # the following will ignore already-encoded elements
      code_el.inner_html = code_el.inner_html.gsub(/&(?!\w+;)/, '&amp;').gsub(/["><]/) { |special| { '"' => '&quot;', '>' => '&gt;', '<' => '&lt;' }[special] }
    end
  end
