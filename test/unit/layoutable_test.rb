...
class LayoutableTest < ActiveSupport::TestCase
  ...
  def test_layout
    ...
    assert_equal 'users/default/', users(:colin).layout_path
    assert_equal 'users/default/default', users(:colin).layout_file
    
    users(:colin)[:layout] = 'msm'
    assert_equal 'msm', users(:colin).layout
    assert_equal 'users/msm/', users(:colin).layout_path
    assert_equal 'users/msm/msm', users(:colin).layout_file
    assert_equal 'users/msm/articles', users(:colin).layout_file(:articles)
  end
