...
class EventsControllerTest < ActionController::TestCase
  ...
  def test_show
    get :show, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:colin).id }
    assert_response :success
    assert_equal events(:birthday), assigns(:event)
  end
  
  def test_show_without_user_nick
    assert_raise(ActionController::RoutingError) { get :show, { :id => events(:birthday).id }, { :user_id => users(:colin).id } }
  end
  ...
  def test_show_without_login_on_protected
    ...
    get :show, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:keira).id }
    assert_redirected_to user_url(users(:keira))
    assert_equal "You are not authorized for that action.", flash[:warning]
  end
  
  def test_new
    get :new, { :user_id => users(:colin).nick }, { :user_id => users(:colin).id }
    assert_response :success
    assert_not_nil assigns(:event)
  end
  
  def test_new_with_diff_user
    get :new, { :user_id => users(:colin).nick }, { :user_id => users(:nana).id }
    assert_redirected_to new_user_event_url(users(:nana))
  end
  
  def test_new_with_non_user
    get :new, { :user_id => users(:colin).nick }, { :user_id => 12312312312 }
    assert_redirected_to login_url
    assert_equal "You need to be logged in to do that.", flash[:warning]
  end
  
  def test_create
    post :create, { :user_id => users(:colin).nick, :event => { :name => 'brouhaha', :content => 'hahaha.brouhahah.', :user_id => users(:colin).id } }, { :user_id => users(:colin).id }
    assert_not_nil assigns(:event)
  ...
  def test_create_with_diff_user
    post :create, { :user_id => users(:colin).nick, :event => { :name => 'brouhaha', :content => 'hahaha.brouhahah.', :user_id => users(:keira).id } }, { :user_id => users(:keira).id }
    assert_not_nil assigns(:event)
  ...
  def test_create_with_non_user
    post :create, { :user_id => users(:colin).nick, :event => { :name => 'brouhaha', :content => 'hahaha.brouhahah.', :user_id => users(:colin).id } }, { :user_id => 1213231231 }
    assert_redirected_to login_url
    assert_equal "You need to be logged in to do that.", flash[:warning]
  end
  
  def test_create_with_invalid_entry
    post :create, { :user_id => users(:colin).nick, :event => { :name => '', :content => 'hahaha.brouhahah.', :user_id => users(:colin).id } }, { :user_id => users(:colin).id }
    assert_flash_equal "There was an error creating your event.", :warning
    assert !assigns(:event).valid?
  end
  
  def test_edit
    get :edit, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:colin).id }
    assert_response :success
    assert_not_nil assigns(:event)
    assert_equal events(:birthday), assigns(:event)
  end
  ...
  def test_edit_by_non_allowed
    get :edit, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:alessandra).id }
  ...
  def test_update
    put :update, { :user_id => users(:colin).nick, :id => events(:birthday).id, :event => { :content => "birthday bash!" } }, { :user_id => users(:colin).id }
  ...
  def test_update_by_unauthorized
    put :update, { :user_id => users(:colin).nick, :id => events(:birthday).id, :event => { :content => "birthday bash!" } }, { :user_id => users(:keira).id }
    assert_redirected_to user_url(users(:keira))
    assert_equal "You are not authorized for that action.", flash[:warning]    
  end
  
  def test_update_with_wrong_nick
    put :update, { :user_id => users(:colin).nick.chop, :id => events(:birthday).id, :event => { :content => "birthday bash!" } }, { :user_id => users(:colin).id }
    assert_template 'error'
    assert_flash_equal "That User entry could not be found. Please check the address.", :warning
  end
  
  def test_begin_event
    put :begin_event, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:colin).id }
    assert_redirected_to user_event_url(users(:colin), events(:birthday))
    assert_equal "Your event #{events(:birthday).display_name} has officially begun!", flash[:notice]
  end
  
  def test_end_event
    put :end_event, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:colin).id }
    assert_redirected_to user_event_url(users(:colin), events(:birthday))
  ...
  def test_destroy
    ...
    delete :destroy, { :user_id => users(:colin).nick, :id => events(:birthday).id }, { :user_id => users(:colin).id }
