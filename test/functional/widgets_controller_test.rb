...
class WidgetsControllerTest < ActionController::TestCase
  ...
  def test_place
    put :place, widgets(:colin_clip).to_path.merge(:widget => { :position => 4 }), as(:colin)
    assert_redirected_to user_widgets_url(users(:colin))
    assert_equal "You have placed #{widgets(:colin_clip).display_name}.", flash[:notice]
  end
  
  def test_place_without_params
    put :place, widgets(:colin_clip).to_path, as(:colin)
    assert_template 'error'
    assert_flash_equal "Invalid request. Please try again.", :warning
  end
  
  def test_unplace
    put :unplace, widgets(:colin_clip).to_path, as(:colin)
    assert_redirected_to user_widgets_url(users(:colin))
